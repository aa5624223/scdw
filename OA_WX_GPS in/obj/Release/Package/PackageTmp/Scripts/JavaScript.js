...
Page({
    ...
    data: {
        ...
        FileNameIdex: 0,
        FilePointLimit: 400,//一个文件最多允许存400个点
        disMin: 80,//两点距离超过*才会被记录
        accuracyMin: 600,//精度*以下才会记录
        //需要传给服务器的变量
        RequestId: -1,
        PersonId: -1,
        ccqsrq: '',
    ...
    LocateListenBegin: function () {
        ...
        _locationChangeFn = function (res) {
            if (_this.data.MinAccuracy > res.accuracy) {
                var la = parseFloat(res.latitude).toFixed(8);
                var lo = parseFloat(res.longitude).toFixed(8);
                var ac = res.accuracy;
            ...
            if (_this.data.TimeTemp >= _this.data.MinTime) {
                _this.LogInsert("插入点:" + _this.data.MinPoint.la + "," + _this.data.MinPoint.lo + " 距离:" + 0 + " 时间:" + time);
